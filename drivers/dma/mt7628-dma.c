/*
 * Copyright(c) 2016, Qin Wei <me@vocore.io>
 * MT7628 GDMA support
 *
...
{
	return container_of(chan->vchan.chan.device, struct gdma_dma_dev, ddev);
}
...
static int gdma_dma_terminate_all(struct dma_chan *c)
{
	...
	if(chan->desc) {
		gdma_dma_desc_free(&chan->desc->vdesc);
		chan->desc = NULL;
	}
...
static irqreturn_t gdma_dma_irq(int irq, void *devid)
{
	...
	// TODO: if unmask failed, we also need to free desc.
...
static struct dma_chan *gdma_of_dma_xlate_by_chan_id(
...
static int gdma_dma_probe(struct platform_device *pdev)
{
	...
	ret = of_dma_controller_register(pdev->dev.of_node,
		gdma_of_dma_xlate_by_chan_id, dma_dev);
	if (ret)
		goto err_unregister;
...
MODULE_DESCRIPTION("MT7628 GDMA driver");
MODULE_LICENSE("GPL");
